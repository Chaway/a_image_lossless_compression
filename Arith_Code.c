...
int arith_code(char * args,FILE* fpo_in)
{
  buffer = 0;
  symbol_num = 0;
  ...
  if((fp = fopen(args , "r"))== NULL)    //open symbol file
  {
    printf("%s\n",args);
 ...
 */
 ...
 while( (temp = fgetc(fp)) != EOF)
  ...
  for (int i = 0; i < symbol_num; i++)
  {
    ...
    code_length += sizeof(Symbols[i]) + sizeof(Count[i]);
    //fprintf(fs, "%d %d\n", Symbols[i] , Count[i]);
  }
...
int current_num = 0;
  ...
  fclose(fp);
  //printf("code_length = %d ,encode finished\n",code_length);
  return code_length;
}
